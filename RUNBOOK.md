...
| Date | Change |
|------|--------|
| 2026-08-27 | **Fixed the missing August long read.** Sema's Aug 3 post #319 (market May→early July, 4 Magnificent-7 chart screenshots) had been auto-filed as a *setup* because its text is only 69 words — under `LONGREAD_MIN_WORDS = 150`. The charts carried the substance, so it belonged in long reads. Moved to `src/content/longreads/2026-08-tg319.md` (verbatim text, real date 2026-08-03, photos + TG footer kept; body reflowed to drop the scraper's empty album markup, title/description authored). August now shows 2 long reads (#319, #328). **Root-cause fix in `autopublish.mjs`:** new image-heavy exception — ≥3 photos + ≥55 words + a finance keyword ⇒ long read (`LONGREAD_PHOTO_MIN` / `LONGREAD_PHOTO_MIN_WORDS`), so chart-driven posts stop landing in setups. Everything else in the pipeline was healthy: CI ran daily all month, ingest is caught up through #332, and the other August posts were classified correctly (#323 "Pieces of life", #329 meme, #331/#332 videos = correctly skipped; #330 Kobeissi link = correctly a setup). |
