...
| Date | Change |
|------|--------|
| 2026-08-27 | **Published the 2nd August long read (#332, Aug 26) — "Немного о ситуации на рынках".** Why it never auto-published: Telegram's public preview renders that message as *"Please open Telegram to view this post"* and exposes **no text and no media URLs**, while the desktop app shows the full post. Trigger unconfirmed — the message contains a custom/premium Pepe emoji and is marked "edited"; neither is proven to be the cause. What IS measurable: a fresh scrape returns 0 characters of text for #332 while every other August post returns its text (#318 370, #319 263, #328 3651, #330 176, #331 55). `/s/`, `?embed=1`, the post page and its OG tags all return zero text, so ingest saw an empty post and autopublish skipped it as media-only. **Recovery procedure that worked (reuse it):** `open "tg://resolve?domain=investsyoma&post=<id>"` to jump the desktop app to the post, then `screencapture -x -o` (needs `dangerouslyDisableSandbox`) and read the crops. The Telegram window is small and the pinned-message bar overlays the top of the chat — jumping to a *neighbouring* post and burst-capturing 8–10 frames during the scroll animation gets the parts that don't fit on one screen. Text transcribed verbatim (his `-` separators escaped as `\-` so markdown doesn't turn them into bullets); the portfolio donut (BTC 77.4 / ETH 11.8 / HYPE 4.7 / AAVE 2.3 / BNB 1.5 / PEPE 1.1 / JUP 0.6 / PUMP 0.5) cropped from the capture to `public/images/tg/332.jpg` with his caption "Мой потфель на 26/08/26". The custom Pepe emoji has no Unicode equivalent and was dropped. August now has 2 long reads (#328, #332). **Pipeline hardened:** `ingest.mjs` sets `unreadable` on posts with the `message_media_not_supported` block and no text; `autopublish.mjs` holds those in `seen.blocked` for `RETRY_DAYS = 14` and logs `! #<id> NEEDS ATTENTION` instead of silently marking them handled. Also confirmed the rest of August is correct: #319 (Aug 3) is a setup, #330 a setup, #323/#329/#331 correctly skipped.
